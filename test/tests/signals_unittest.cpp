...
TEST_CASE("Connect functions to a signal", "[Signal]")
{
    Foo foo("kc");
    Signal<int, std::string&&> worker_signal;
...
TEST_CASE("Disconnect via slots", "[Signal]")
{
    // In order to destroy signal manually.
    auto worker_signal = std::make_unique<Signal<int, std::string&&>>();
...
TEST_CASE("Its safe when bound objects are gone", "[Signal]")
{
    Signal<int, std::string&&> worker_signal;
