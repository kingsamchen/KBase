...
TEST_CASE("Traits can be varied", "[Singleton]")
{
    SECTION("use default traits")
    {
        kbase::AtExitManager exit_manager;
        auto i = kbase::Singleton<int>::instance();
        *i = 0x12345678;

        REQUIRE(123 == SingletonUser::Instance()->n);
    }

    SECTION("use leaky traits")
    {
        LeakySingletonUser::instance()->Greet();
    }

    SECTION("use custom traits")
    {
        CustomSingleton::instance()->DumpMessage();
    }
}

TEST_CASE("Ensure thread safety", "[Singleton]")
{
    AtExitManager exit_manager;
    std::vector<std::thread> vth;
    for (int i = 0; i < 100; ++i) {
        vth.emplace_back([](int id) {
            auto ptr = Singleton<DataPack>::instance();
